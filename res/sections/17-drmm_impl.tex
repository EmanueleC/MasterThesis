...
\begin{tikzpicture}[node distance = 3.7cm, auto]
    ...
    \node [block, right of=hist] (ids) {Generate positive/\\negative samples for training};
...
DRMM is not the first (Neural) IR model to evaluate its performance on TREC Robust 2004 track (\cite{rob04}). In fact, during past years, many retrieval models have been tested against this collection.
...
Anserini BM25+RM3 \cite{neurhype} & 0.290 & \textit{Not available} & \textit{Not available} & 2018 \\ \hline
Anserini QL+RM3 \cite{neurhype} & 0.274 & \textit{Not available} & \textit{Not available} & 2018 \\ \hline
SNRM \cite{stdlnneur} & 0.285 & 0.376 & 0.431 & 2018 \\ \hline
SNRM-PRF \cite{stdlnneur} & 0.297 & 0.394 & 0.439 & 2018 \\ \hline
NPRF-DRMM \cite{nprf} & 0.290 & 0.406 & 0.45 & 2018 \\ \hline
DRMM \cite{drmm} & 0.279 & 0.382 & 0.431 & 2017 \\ \hline
\end{tabular}
...
\begin{figure}[H]
  ...
  \caption{Count-based histograms}
  \label{fig:hist_ex}
\end{figure}
...
When using my processed corpus, the vocabulary size increases as well as the documents count. All of the metrics descreases as well, whether I use my embeddings or the originals. Thus, my corpus seems to influence the performances more than the embeddings.
...
So, even if the collection test, the architecture of the neural network and the (declared) methodology were the same, it is still very difficult to obtain close results.
